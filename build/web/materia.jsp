...
<jsp:include page="WEB-INF/cabezera.jsp" />
        ...
        <p><a href="MateriaControlador?action=add" class="btn"><img src="Imagenes/nuevo.png" title="Nuevo registro"/>Nuevo</a></p>
        <div class="datagrid">
        <table border="2">
            <thead>
            <tr>
            ...
            <c:forEach var="item" items="${materias}">
                 <tr>
                    ...
                    <td>${item.sigla}</td>
                    <td>${item.nivel}</td>
                    <th><a href="MateriaControlador?action=edit&id=${item.id}"><img src="Imagenes/editar.png" title="Modificar"/></a></th>
                    <td><a href="MateriaControlador?action=delete&id=${item.id}" onclick="return(confirm('Esta seguro de eliminar??'))">
